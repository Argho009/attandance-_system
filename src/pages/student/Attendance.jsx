...
export const StudentAttendance = () => {
  ...
  const fetchData = async () => {
    ...
    const { data: lectures } = await supabase.from('lectures').select('id, subject_id, date, lecture_no').eq('sem', stu.sem).eq('is_skipped', false).order('date');
    const { data: attendance } = await supabase.from('attendance').select('lecture_id, status, remarks').eq('student_id', stu.id);
    ...
    const summary = (subjects || []).map(sub => {
      ...
      const subAtt = (attendance || []).filter(a => subLectures.some(l => l.id === a.lecture_id));
  ...
  const handleViewDetail = (sub) => {
    ...
    const detail = sub.lectures.map(l => ({
      date: l.date,
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-4">
          <button onClick={() => setSelectedSubject(null)} className="text-sm text-indigo-600 hover:underline">← Back</button>
          <div>
            <h2 className="text-xl font-semibold text-slate-800">{selectedSubject.code} — {selectedSubject.name}</h2>
            <p className="text-sm text-slate-500">
              ...
              {selectedSubject.condoned > 0 && <span className="ml-2 text-blue-600">({selectedSubject.condoned} condoned)</span>}
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {lectureDetail.map((l, i) => (
                <tr key={i} className={`border-b last:border-0 ${l.status === 'present' ? 'bg-green-50/30' : 'bg-red-50/30'}`}>
                  <td className="px-4 py-3">{new Date(l.date).toLocaleDateString('en-IN', { weekday: 'short', day: '2-digit', month: 'short' })}</td>
                  <td className="px-4 py-3">{l.lectureNo}</td>
                  <td className="px-4 py-3">
                    <Badge variant={l.status === 'present' ? 'success' : 'danger'}>{l.status}</Badge>
                  </td>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Subject-wise attendance. Click on a subject to see lecture-by-lecture detail.</p>
      </div>
        ...
        {subjectData.map(sub => (
          <div key={sub.id}
            ...
            <div className="flex-1">
              <p className="font-semibold text-slate-800">{sub.code} <span className="font-normal text-slate-500">— {sub.name}</span></p>
              ...
              <p className="text-xs text-slate-400 mt-1">{sub.present}/{sub.total} attended{sub.condoned > 0 ? ` + ${sub.condoned} condoned` : ''}</p>
            </div>
